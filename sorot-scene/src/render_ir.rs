...
use sorot_core::math::{Matrix3x2, Rect};
...
/// A screen-space tile containing packets that intersect it.
#[derive(Debug, Clone)]
...
impl RenderFrame {
    ...
    pub fn bin_packet(&mut self, packet: RenderPacket) {
        ...
        let end_col = ((clip.max.x.min(self.screen_width as f32) / ts).ceil() as u32)
            .min(self.cols());
        let end_row = ((clip.max.y.min(self.screen_height as f32) / ts).ceil() as u32)
            .min(self.rows());

        for row in start_row..end_row {
            for col in start_col..end_col {
                self.tile_at_mut(col, row).packets.push(packet.clone());
            }
        }
    }
}
